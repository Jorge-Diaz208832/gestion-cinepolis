...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<title>Recetario de Baguettes</title>
...
int cveBaguis = request.getParameter("xCveBaguis")!=null?
Integer.parseInt(request.getParameter("xCveBaguis")):0;
...
{
	linea="select * from baguis where cve_baguis=" + clave;
...
if(accion.equals("guardarcerrar") || accion.equals("guardarNuevo"))
{
	...
	{
		clave = String.valueOf(UtilDB.getSiguienteNumero("baguis","cve_baguis"));

		linea="insert into baguis(cve_baguis, nombre, descripcion, elaboracion) values (" + clave + ", " +
	...
	{
		UtilDB.ejecutaSQL("delete from baguis where cve_baguis=" + clave);
		accion="listado";
	}
...
<script>
	...
	function validate(accion) 
	{
		...
		{
			alert("Debe escribir el nombre del bagui");
			forma.xNombre.focus();
			errores++;
		}
		...
		else 
		{
			if (forma.xDescripcion.value=="")
				{
					alert("Debe escribir la descripcion del bagui");
			...
			<td>
				<table width="95%" align="center" cellpadding="0" cellspacing="0" border="1">
					<tr bgcolor="black">
						...
						Recetario de Baguis Cinepolis</td></h2>
					</tr>
					<% 
					...
					<tr>
						<td colspan="5">
							...
							<%
							 rs = UtilDB.ejecutaConsulta("Select e.*, e.nombre as Baguis from baguis as e order by e.nombre");
													renglon=0;
													while(rs.next())
													{
														renglon++;
							%>
								...
								<tr bgcolor="white" align="center">
									...
									<td class="celdaEncabezado" align="center">
										<a href="#" onclick="recargar('<%=rs.getInt("cve_baguis")%>')"><img align="center" src="imagenes/edit.png" width="20" alt=""></a>
									</td>
					...
					<tr>
						<td colspan="5">
							<table>
									<tr>
										<td style="color:#FFFFFF" align="right"><h4>Datos del Bagui</h4></td>
										<td></td>
									</tr>
									<tr>
										<td></td>
