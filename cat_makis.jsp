...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<title>Recetario de Makis</title>
...
int cveMakis = request.getParameter("xCveMakis")!=null?
Integer.parseInt(request.getParameter("xCveMakis")):0;
...
{
	linea="select * from makis where cve_makis=" + clave;
...
if(accion.equals("guardarcerrar") || accion.equals("guardarNuevo"))
{
	...
	{
		clave = String.valueOf(UtilDB.getSiguienteNumero("makis","cve_makis"));

		linea="insert into makis(cve_makis, nombre, descripcion, elaboracion) values (" + clave + ", " +
	...
	{
		UtilDB.ejecutaSQL("delete from makis where cve_makis=" + clave);
		accion="listado";
	}
...
<script>
	...
	function validate(accion) 
	{
		...
		{
			alert("Debe escribir el nombre del maki");
			forma.xNombre.focus();
			errores++;
		}
		...
		else 
		{
			if (forma.xDescripcion.value=="")
				{
					alert("Debe escribir la descripcion del maki");
			...
			<td>
				<table width="95%" align="center" cellpadding="0" cellspacing="0" border="1">
					<tr bgcolor="black">
						...
						Recetario de Makis Cinepolis</td></h2>
					</tr>
					<% 
					...
					<tr>
						<td colspan="5">
							...
							<%
							 rs = UtilDB.ejecutaConsulta("Select e.*, e.nombre as Makis from makis as e order by e.nombre");
													renglon=0;
													while(rs.next())
													{
														renglon++;
							%>
								...
								<tr bgcolor="white" align="center">
									...
									<td class="celdaEncabezado" align="center">
										<a href="#" onclick="recargar('<%=rs.getInt("cve_makis")%>')"><img align="center" src="imagenes/edit.png" width="20" alt=""></a>
									</td>
					...
					<tr>
						<td colspan="5">
							<table>
									<tr>
										<td style="color:#FFFFFF" align="right"><h4>Datos del Maki</h4></td>
										<td></td>
									</tr>
									<tr>
										<td></td>
