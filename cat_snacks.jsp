...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<title>Recetario de Snacks</title>
...
int cveSnacks = request.getParameter("xCveSnacks")!=null?
Integer.parseInt(request.getParameter("xCveSnacks")):0;
...
{
	linea="select * from snacks where cve_snacks=" + clave;
...
if(accion.equals("guardarcerrar") || accion.equals("guardarNuevo"))
{
	...
	{
		clave = String.valueOf(UtilDB.getSiguienteNumero("snacks","cve_snacks"));

		linea="insert into snacks(cve_snacks, nombre, descripcion, elaboracion) values (" + clave + ", " +
	...
	{
		UtilDB.ejecutaSQL("delete from snacks where cve_snacks=" + clave);
		accion="listado";
	}
...
<script>
	...
	function validate(accion) 
	{
		...
		{
			alert("Debe escribir el nombre del Snack");
			forma.xNombre.focus();
			errores++;
		}
		...
		else 
		{
			if (forma.xDescripcion.value=="")
				{
					alert("Debe escribir la descripcion del Snack");
			...
			<td>
				<table width="95%" align="center" cellpadding="0" cellspacing="0" border="1">
					<tr bgcolor="black">
						...
						Recetario de Snacks Cinepolis</td></h2>
					</tr>
					<% 
					...
					<tr>
						<td colspan="5">
							...
							<%
							 rs = UtilDB.ejecutaConsulta("Select e.*, e.nombre as Snacks from Snacks as e order by e.nombre");
													renglon=0;
													while(rs.next())
													{
														renglon++;
							%>
								...
								<tr bgcolor="white" align="center">
									...
									<td class="celdaEncabezado" align="center">
										<a href="#" onclick="recargar('<%=rs.getInt("cve_snacks")%>')"><img align="center" src="imagenes/edit.png" width="20" alt=""></a>
									</td>
					...
					<tr>
						<td colspan="5">
							<table>
									<tr>
										<td style="color:#FFFFFF" align="right"><h4>Datos del Snack</h4></td>
										<td></td>
									</tr>
									<tr>
										<td></td>
									...
									<tr>
										...
										<td> <input type="text" id="xNombre" name="xNombre" size="40" maxlength="100" value="<%=nombre%>"></td>
									</tr>
									<tr>
